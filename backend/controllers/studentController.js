...
const { computeRegistrationStages, parseStudentData, aggregateRegistrationOverallFromStudents, computeRegistrationGroupAggregates, enrichRegistrationAbstractRows, hasOptionalRegistrationStages, buildRegistrationStatusLabelMap, resolveOptionalStagesForStudent } = require('../services/registrationStages');
...
const resolveOptionalStages = (stageConfig, branchCodeOrStudent, studentYear = null) => {
  // Student-aware call: resolveOptionalStages(config, student)
  if (branchCodeOrStudent && typeof branchCodeOrStudent === 'object') {
    return resolveOptionalStagesForStudent(stageConfig, branchCodeOrStudent, studentYear);
  }
  // Legacy (branch, year) — no batch available, keep config lookup (used only where student is passed separately)
  return resolveOptionalStagesFromConfig(stageConfig, branchCodeOrStudent, studentYear);
};
...
/**
 * Filter registration report rows to students pending a given stage.
 * Matches the pending definitions used in registration report statistics.
 */
const buildRegistrationStagePendingFilterClause = (stage, academicYearFromYear = null) => {
  ...
  switch (normalized) {
    case 'registration':
    case 'overall': {
      const completedSql = buildRegistrationOverallCompletedSql(
        alias,
        verificationCompletedSql,
        academicYearFromYear
      );
      const temporarySql = buildRegistrationOverallTemporarySql(
        alias,
        verificationCompletedSql,
        academicYearFromYear
      );
      return ` AND NOT (${completedSql}) AND NOT (${temporarySql})`;
    }
    case 'verification':
      return ` AND NOT (${verificationCompletedSql})`;
    case 'certificates':
      return ` AND NOT (${certificatesVerifiedSql(alias)}) AND NOT (${certificatesTemporarySql(alias)})`;
    case 'fees':
    case 'fee':
      return ` AND NOT (${feeClearedSql(alias)})`;
    case 'promotion':
      return ` AND NOT (${promotionCompletedSql})`;
    default:
      return '';
  }
};
...
const buildRegistrationStatsCacheKey = (req) => crypto.createHash('md5').update(JSON.stringify({
  ...
  filter_stage_pending: req.query.filter_stage_pending || '',
...
const syncRegistrationStatusColumn = async (admissionNumber, statusLabel) => {
  const hasRegColumn = await columnExists('registration_status');
  if (!hasRegColumn || !admissionNumber) return;
  await masterPool.query(
    'UPDATE students SET registration_status = ? WHERE admission_number = ?',
    [statusLabel, admissionNumber]
  );
  clearStudentsCache();
};
...
exports.getAllStudents = async (req, res) => {
  try {
    ...
    const registrationStatusFilter = normalizedOtherFilters.filter_registration_status;
    if (registrationStatusFilter && typeof registrationStatusFilter === 'string' && registrationStatusFilter.trim().length > 0) {
      const normalizedReg = registrationStatusFilter.trim().toLowerCase();
      const regStatusMatch = normalizedReg === 'completed'
        ? 'Completed'
    ...
    const registrationStatusFilterForCount = normalizedOtherFilters.filter_registration_status;
    if (registrationStatusFilterForCount && typeof registrationStatusFilterForCount === 'string' && registrationStatusFilterForCount.trim().length > 0) {
      const normalizedReg = registrationStatusFilterForCount.trim().toLowerCase();
      const regStatusMatch = normalizedReg === 'completed'
        ? 'Completed'
        : normalizedReg === 'temporary'
          ? 'Temporary'
...
exports.getStudentByAdmission = async (req, res) => {
  try {
    ...
    const optionalStagesSingle = resolveOptionalStages(
      stageConfigSingle,
    ...
    if (storedReg !== computedRegistrationStatus.toLowerCase()) {
      syncRegistrationStatusColumn(admissionNumber, computedRegistrationStatus).catch((err) => {
        console.warn('Registration status sync failed:', err.message);
      });
    }
    ...
    const responsePayload = {
      ...
      registration_status: computedRegistrationStatus,
      scholar_status: normalizedScholarStatus
    };
  ...
  try {
    ...
    const [rows] = await masterPool.query(
      `SELECT id, student_data, certificates_status, fee_status, scholar_status,
              current_year, current_semester, registration_status, batch, stud_type, branch
    ...
    if (statusLabel === 'Completed' || statusLabel === 'Temporary') {
      if (hasRegColumn && columnReg.toLowerCase() !== statusLabel.toLowerCase()) {
        await masterPool.query(
          'UPDATE students SET registration_status = ? WHERE admission_number = ?',
          [statusLabel, admissionNumber]
        );
      ...
      await syncRegistrationJson(statusLabel);
    }
  } catch (err) {
...
exports.getRegistrationReport = async (req, res) => {
  try {
    ...
    */
    ...
    // Stage pending drill-down (mirrors stats card pending counts)
    const stagePendingFilter = (filter_stage_pending || '').trim().toLowerCase();
    baseQuery += buildRegistrationStagePendingFilterClause(stagePendingFilter, academicYearFromYear);
...
const mapRegistrationAbstractRowToExcel = (row) => {
  ...
  const promotionCompleted = parseInt(row.promotion_completed || 0, 10);
  const scholarshipAssigned = parseInt(row.scholarship_assigned || 0, 10);
  const scholarshipPending = parseInt(
    row.scholarship_pending ?? (total - scholarshipAssigned),
    10
  );
  ...
  return {
    ...
    Promotion: ratio(promotionCompleted),
    Scholarship: `${scholarshipAssigned}/${scholarshipPending}`
  };
};
    ...
    */
    ...
    const statsQuery = `
      SELECT 
        ...
        SUM(CASE WHEN certificates_status LIKE '%Verified%' OR certificates_status = 'completed' THEN 1 ELSE 0 END) as certificates_verified,
        SUM(CASE WHEN fee_status LIKE '%no_due%' OR fee_status LIKE '%no due%' OR fee_status LIKE '%permitted%' OR fee_status LIKE '%completed%' OR fee_status LIKE '%nodue%' THEN 1 ELSE 0 END) as fee_cleared,
        SUM(${buildRegistrationOverallCompletedCaseSql('students', verificationCompletedSql, academicYearFromYearExport)}) as overall_completed,
    ...
    const statistics = {
      total: totalCount,
      verification: { completed: parseInt(statsRow.verification_completed || 0, 10) },
    ...
    const processedData = students.map((student) => {
      const studentData = parseStudentData(student);
      const optionalStages = resolveOptionalStages(stageConfigExport, student);
